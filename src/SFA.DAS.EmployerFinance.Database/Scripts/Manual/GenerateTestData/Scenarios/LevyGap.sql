...
-- SCENARIO: Levy Gap
--
-- An employer account where levy declarations stop for a period mid-history
-- then resume. GapFrom and GapTo are monthBeforeToDate offsets (negative
-- integers), e.g. -9 and -4 means no declarations from 9 months ago to 4
-- months ago relative to today.
--
-- Requires SQLCMD mode: Query > SQLCMD Mode in SSMS, or natively in Azure Data Studio
-- =============================================================================

-- ===================== CONFIGURATION - CHANGE THESE ========================
:setvar AccountId         400
:setvar PayeScheme        "400/LG00001"
:setvar AccountName       "Levy Gap Co"
:setvar MonthlyLevy       1000
:setvar LevyAllowance     1500
:setvar NumberOfMonths    25
:setvar GapFrom           -9
:setvar GapTo             -4
:setvar MonthlyPayments   300
:setvar PaymentsPerMonth  2
-- ===========================================================================
...
BEGIN TRANSACTION ScenarioLevyGapLevies
GO
...
DECLARE @gapFrom                int             = $(GapFrom)
DECLARE @gapTo                  int             = $(GapTo)
...
BEGIN
    DECLARE @offset int = -@numberOfMonthsToCreate + @monthNumber
    IF @offset NOT BETWEEN @gapFrom AND @gapTo
    BEGIN
        INSERT INTO @levyDecByMonth (monthBeforeToDate, amount, createMonth, payrollYear, payrollMonth)
        VALUES (
            @offset,
            (CASE
                WHEN dbo.PayrollYear(DATEADD(month, -1+@offset, @toDate)) = @firstPayrollYear
                    THEN @monthlyLevy * @monthNumber
                    ELSE @monthlyLevy * dbo.PayrollMonth(DATEADD(month, -1+@offset, @toDate))
            END),
            DATEADD(month, @offset, @toDate),
            dbo.PayrollYear(DATEADD(month,  -1+@offset, @toDate)),
            dbo.PayrollMonth(DATEADD(month, -1+@offset, @toDate)))
    END
...
COMMIT TRANSACTION ScenarioLevyGapLevies
