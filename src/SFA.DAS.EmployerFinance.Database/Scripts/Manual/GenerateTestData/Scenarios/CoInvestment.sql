-- =============================================================================
-- SCENARIO: Co-Investment
--
-- A small levy-paying employer whose levy pot is insufficient to cover all
-- apprenticeships. Each month produces two payment records:
--   - 1 apprentice fully levy-funded        (FundingSource=1)
--   - 1 apprentice co-funded: SFA (FS=2) + employer (FS=3)
-- Both are grouped into a single TransactionLine per period showing the split.
--
-- Requires SQLCMD mode: Query > SQLCMD Mode in SSMS, or natively in Azure Data Studio
-- =============================================================================

-- ===================== CONFIGURATION - CHANGE THESE ========================
:setvar AccountId            500
:setvar PayeScheme           "500/CI00001"
:setvar AccountName          "CoInvest Ltd"
:setvar MonthlyLevy          200
:setvar LevyAllowance        300
:setvar NumberOfMonths       25
:setvar LevyPaymentAmount    100
:setvar SfaCoInvestAmount    120
:setvar EmpCoInvestAmount    30
-- ===========================================================================
...
BEGIN TRANSACTION ScenarioCoInvestmentLevies
GO
...
COMMIT TRANSACTION ScenarioCoInvestmentLevies
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM employer_financial.PeriodEnd WHERE PeriodEndId = @id)
        ...
        VALUES (@id, dbo.CalendarPeriodMonth(@periodEndDate), dbo.CalendarPeriodYear(@periodEndDate),
                ...
                'https://manual-data-payments.apprenticeships.gov.uk/api/payments?periodId=' + @id)
END
GO
...
CREATE OR ALTER PROCEDURE #createPaymentsForMonth (
    @accountId bigint, @createDate datetime,
    @levyAmount decimal(18,5), @sfaAmount decimal(18,5), @empAmount decimal(18,5)
) AS
BEGIN
BEGIN TRANSACTION
    ...
    -- Apprentice 1: fully levy-funded
    EXEC #createPayment @accountId, 'CHESTERFIELD COLLEGE', 'Accounting', 1,
        'A Apprentice', @ukprn, 1000000001, 2001, /*Levy*/1, @levyAmount, @periodEndDate

    -- Apprentice 2: co-funded (SFA contribution + employer contribution)
    EXEC #createPayment @accountId, 'CHESTERFIELD COLLEGE', 'Project Management', 1,
        'B Apprentice', @ukprn, 1000000002, 2002, /*SFA co-invest*/2, @sfaAmount, @periodEndDate
    EXEC #createPayment @accountId, 'CHESTERFIELD COLLEGE', 'Project Management', 1,
        'B Apprentice', @ukprn, 1000000002, 2002, /*Employer co-invest*/3, @empAmount, @periodEndDate
...
DECLARE @numberOfMonthsToCreate int             = $(NumberOfMonths)
DECLARE @levyPaymentAmount      decimal(18,5)   = $(LevyPaymentAmount)
DECLARE @sfaCoInvestAmount      decimal(18,5)   = $(SfaCoInvestAmount)
DECLARE @empCoInvestAmount      decimal(18,5)   = $(EmpCoInvestAmount)
...
BEGIN
    ...
    IF @@ROWCOUNT = 0 BREAK
    EXEC #createPaymentsForMonth @accountId, @createDate, @levyPaymentAmount, @sfaCoInvestAmount, @empCoInvestAmount
END
