...
namespace SFA.DAS.EmployerFinance.UnitTests.Formatters.Transactions
{
    public class LevyCsvTransactionFormatterTests : BaseFormatterTest
    {
        ...
        {
            var formattedFileData = PaymentFormatter.GetFileData(TransactionDownloadLines, false);
        ...
        [Test]
        public void WhenICallGetContentsIGetCorrectHeaderFormatForNewVersion()
        {
            var formattedFileData = PaymentFormatter.GetFileData(TransactionDownloadLines, true);
            ...
            headerColumns[14].Should().Be("Course type");
            headerColumns[15].Should().Be("Paid from levy");
            headerColumns[16].Should().Be("Paid from transfer");
            headerColumns[17].Should().Be("Your contribution");
            headerColumns[18].Should().Be("Government contribution");
            headerColumns[19].Should().Be("Total");
        }
        ...
        [Test]
        public void WhenICallGetContentsIGetCorrectDataFormatForNewVersion()
        {
            var formattedFileData = PaymentFormatter.GetFileData(TransactionDownloadLines, true); 
            ...
            while (i < 4)
            {
                ...
                dataRow[16].Should().Be((0).ToString("0.00000", CultureInfo.CurrentCulture));
                dataRow[17].Should().Be((i).ToString("0.00000", CultureInfo.CurrentCulture));
                dataRow[18].Should().Be((i * 10000).ToString("0.00000", CultureInfo.CurrentCulture));
                dataRow[19].Should().Be(((i * 1000) + (i * 100)).ToString("0.00000", CultureInfo.CurrentCulture));
                i++;
            }
        }
    }
