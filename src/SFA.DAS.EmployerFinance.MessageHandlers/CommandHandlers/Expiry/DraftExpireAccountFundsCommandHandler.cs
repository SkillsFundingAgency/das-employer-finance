...
public class DraftExpireAccountFundsCommandHandler : IHandleMessages<DraftExpireAccountFundsCommand>
{
    ...
    public async Task Handle(DraftExpireAccountFundsCommand message, IMessageHandlerContext context)
    {
        ...
        {
            await _expiredFundsRepository.CreateDraft(message.AccountId,
                shortTermExpiredFunds.Where(c => c.Key.Equals(currentCalendarPeriod))
                    .ToDictionary(key => key.Key, value => value.Value).ToExpiredFundsList().ToList(), now,
                transactionType: 6);
        }
