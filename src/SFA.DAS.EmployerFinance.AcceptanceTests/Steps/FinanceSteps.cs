...
namespace SFA.DAS.EmployerFinance.AcceptanceTests.Steps
{
    ...
    public class FinanceSteps : TechTalk.SpecFlow.Steps
    {
        ...
        public Task ThenLevel1HasRowWithCorrectBalance(int balance, int month, int year)
        {
            ...
            {
                var account = _objectContext.Get<Account>();
                var actual = await c.Resolve<EmployerAccountTransactionsOrchestrator>().GetAccountTransactions(_objectContainer.Resolve<IEncodingService>().Encode(account.Id,EncodingType.AccountId), year, month);
        ...
        public Task ThenUserDaveFromAccountAShouldSeeALevelScreenWithALevyDeclaredOfOnThe(int levyDeclared, int month, int year)
        {
            return _objectContainer.ScopeAsync(async c =>
            {
                ...
                var viewModel = await c.Resolve<EmployerAccountTransactionsOrchestrator>().FindAccountLevyDeclarationTransactions(_objectContainer.Resolve<IEncodingService>().Encode(account.Id, EncodingType.AccountId), fromDate, toDate);
